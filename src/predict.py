...
=========================
Sử dụng model đã train để dự đoán bệnh
"""
...
class HealthPredictor:
    ...
    def predict_single(self, patient_data):
        ...
        predicted_disease = self.trainer.disease_encoder.inverse_transform(prediction)[0]
...
def interactive_input():
    ...
    # Symptoms
    print("\nTriệu chứng (y/n):")
    symptoms = ['fever', 'cough', 'headache', 'sore_throat', 'runny_nose', 'fatigue', 'body_aches']
    symptom_names = ['Sốt', 'Ho', 'Đau đầu', 'Đau họng', 'Sổ mũi', 'Mệt mỏi', 'Đau nhức cơ thể']
    
    for symptom, name in zip(symptoms, symptom_names):
        answer = input(f"{name}: ").lower()
        patient_data[symptom] = 1 if answer.startswith('y') or answer.startswith('c') else 0
    ...
    patient_data['recent_travel'] = 1 if travel.startswith('y') or travel.startswith('c') else 0
    
    contact = input("Tiếp xúc với người bệnh (y/n): ").lower()
    patient_data['contact_sick'] = 1 if contact.startswith('y') or contact.startswith('c') else 0
