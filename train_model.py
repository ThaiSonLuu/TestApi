...
================================
Train model dự báo bệnh dựa trên triệu chứng và thời tiết
"""
...
class HealthPredictionTrainer:
    ...
    def create_sample_data(self):
        ...
        for i in range(n_samples):
            ...
            if (df.iloc[i]['cough'] and df.iloc[i]['runny_nose'] and 
                df.iloc[i]['weather_temp'] < 15):
                diseases.append('Common Cold')
            
            # Flu - fever + body_aches + fatigue
            elif (df.iloc[i]['fever'] and df.iloc[i]['body_aches'] and 
                  df.iloc[i]['fatigue']):
                diseases.append('Seasonal Flu')
            
            # Allergic Rhinitis - runny_nose + season spring/summer + bad air quality
            elif (df.iloc[i]['runny_nose'] and df.iloc[i]['season'] in [1, 2] and 
                  df.iloc[i]['air_quality'] >= 3):
                diseases.append('Allergic Rhinitis')
            
            # Heat Exhaustion - high temp + fatigue + hot weather
            elif (df.iloc[i]['fatigue'] and df.iloc[i]['weather_temp'] > 35 and 
                  df.iloc[i]['headache']):
    ...
    def train(self, data_file=None):
        ...
        if data_file and os.path.exists(data_file):
            ...
            print("💾 Sample data saved to health_data_sample.csv")
    ...
    def save_model(self, filename='health_prediction_model.pkl'):
    ...
    def load_model(self, filename='health_prediction_model.pkl'):
